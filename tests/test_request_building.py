...
class RequestBuildingTest(unittest.TestCase):
    ...
    def test_build_request_uses_developer_message_for_openai_backend(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertNotIn("<permissions_instructions>", request["instructions"])

    def test_build_request_inlines_permissions_for_compatible_backend(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            root: Path = Path(tmp_dir)
            config = AgentConfig(
                model=ModelSettings(
            ...
            self.assertIn("<permissions_instructions>", request["instructions"])
            developer_messages = [
                item
                for item in request["input"]
                if item.get("type") == "message" and item.get("role") == "developer"
            ]
            self.assertEqual(developer_messages, [])
